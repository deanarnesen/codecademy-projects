...
const validExtra = [4, 5, 3, 9, 6, 8, 9, 8, 8, 7, 7, 0, 5, 7, 9, 8]
...
const validateCred = (cardNumberArray) => {
    ...
    for(let i = cardNumberArray.length - 2; i >=0; i -= 2){
        let partialSum = cardNumberArray[i] * 2;
        if (partialSum >= 10) {
            partialSum -= 9;
        }
...
const findInvalidCards = (mysteryArray) => {
    return mysteryArray.filter(card => !validateCred(card))
}


const invalidCardCompanies = (invalidCards) => {
    let fraudulentCompanies = [];
    invalidCards.forEach(card => {
        ...
        if(!fraudulentCompanies.includes(company) && company !== undefined){
            fraudulentCompanies.push(company);
        }
    })
    return fraudulentCompanies;
}

// console.log(validateCred(validExtra))
//
